...
try:
    ...
    cursor.execute("CREATE TABLE employee(employee_id INT NOT NULL, employee_first_name VARCHAR(75) NOT NULL, employee_last_name VARCHAR(75) NOT NULL, employee_role VARCHAR(75) NOT NULL, PRIMARY KEY(employee_id));")
    ...
    cursor.execute("CREATE TABLE sales(distributor_id INT NOT NULL, merlot VARCHAR(75) NOT NULL, cabernet VARCHAR(75) NOT NULL, chablis VARCHAR(75) NOT NULL, chardonnay VARCHAR(75) NOT NULL, CONSTRAINT fk_distributor_id FOREIGN KEY(distributor_id) REFERENCES distributor(distributor_id));")

    #creates a table to hold each employees hours each quarter
    cursor.execute("CREATE TABLE hours(employee_id INT NOT NULL, q1 DECIMAL NOT NULL, q2 DECIMAL NOT NULL, q3 DECIMAL NOT NULL, q4 DECIMAL NOT NULL, CONSTRAINT fk_employee_id FOREIGN KEY(employee_id) REFERENCES employee(employee_id));")
    
    #creates a table with a list of supplies
    cursor.execute("CREATE TABLE supply(supply_id INT NOT NULL, supply_name VARCHAR(75) NOT NULL, supply_price DECIMAL, vendor_id INT NOT NULL, PRIMARY KEY(supply_id), CONSTRAINT fk_supply_vendor_id FOREIGN KEY(vendor_id) REFERENCES vendor(vendor_id));")

    #creates a table for supply orders
    cursor.execute("CREATE TABLE supply_order(order_date DATE NOT NULL, promised_delivery_date DATE NOT NULL, actual_delivery_date DATE, order_price DECIMAL NOT NULL, vendor_id INT NOT NULL, PRIMARY KEY(order_date), CONSTRAINT fk_order_vendor_id FOREIGN KEY(vendor_id) REFERENCES vendor(vendor_id));")
    ...
    while i < len(bacchus["employee"]):
        ...
        cursor.execute(f"INSERT INTO employee(employee_id, employee_first_name, employee_last_name, employee_role) VALUES({employee_id}, {employee_first_name}, {employee_last_name}, {employee_role});")
        i += 1
    ...
    #fills the hours table from the JSON file
    ...
    while i < len(bacchus["supply_order"]):
        order_id = bacchus["supply_order"][i]["order_id"]
        ...
        cursor.execute(f"INSERT INTO supply_order(order_id, order_date, promised_delivery_date, actual_delivery_date, order_price, vendor_id) VALUES({order_id}, {order_date}, {promised_delivery_date}, {actual_delivery_date}, {order_price}, {vendor_id});")
        i += 1
