...
try:
    cursor.execute(f"""CREATE OR REPLACE VIEW sales_by_distributor AS 
                            SELECT sales.distributor_id, SUM(merlot + cabernet + chablis + chardonnay) AS total, ROUND(SUM(merlot + cabernet + chablis + chardonnay) / 4, 0) AS average
                            FROM sales
...
try:
    cursor.execute(f"""CREATE OR REPLACE VIEW employee_hours_total_average AS 
                            SELECT
                                ...
                                ROUND(SUM(q1_hours + q2_hours + q3_hours + q4_hours) / 4, 0) AS average
                            FROM employee
                            WHERE employee_id =1
                        UNION ALL 
                            SELECT
